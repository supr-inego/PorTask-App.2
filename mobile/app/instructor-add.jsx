// FILE: mobile/app/instructor-add.jsx
...
export default function InstructorAddAssignment() {
    ...
    if (!res.canceled) {
      ...
      setAttachments((prev) => [
        ...prev,
        {
          ...
          type: "image",
          name: file.fileName || file.uri?.split("/").pop() || "Image",
          uri: file.uri,
        },
      ]);
    }
  ...
  const handleAdd = async () => {
    if (!title.trim() || !subject.trim() || !description.trim()) {
      Alert.alert(
        "Missing fields",
        "Please fill in title, subject, and description."
      );
      return;
    }
    ...
    // check deadline conflict (only active)
    ...
    try {
      ...
      Alert.alert("Success", "Assignment added!", [
        { text: "OK", onPress: () => router.replace("/(instructorTabs)/home") },
      ]);
    } catch (e) {
...
const styles = StyleSheet.create({
  ...
  container: { paddingTop: 40, paddingBottom: 40, paddingHorizontal: 16 },
  header: {
    fontSize: 22,
    fontWeight: "800",
    marginBottom: 16,
  ...
  attachButtonText: { color: "#1D4ED8", fontWeight: "600" },
  attachmentItem: { flexDirection: "row", alignItems: "center", marginBottom: 8 },
  attachmentThumb: { width: 36, height: 36, borderRadius: 6, marginRight: 8 },
  fileIcon: {
    width: 36,
    height: 36,
    borderRadius: 6,
  ...
  attachmentName: { flex: 1, fontSize: 13, color: "#111827" },
  removeText: { fontSize: 12, color: "#EF4444", marginLeft: 6, fontWeight: "600" },
  submitBtn: {
    marginTop: 22,
  ...
  sheetOption: { paddingVertical: 10 },
